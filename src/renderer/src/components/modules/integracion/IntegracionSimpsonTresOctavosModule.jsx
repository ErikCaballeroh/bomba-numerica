...
export const IntegracionSimpsonTresOctavosModule = (props) => {
  ...
  // Pool de 10 problemas predefinidos (n debe ser múltiplo de 3)
  const problemsPool = [
    {
      function: '1 - x²',
      fnLatex: '1 - x^2',
  ...
  useEffect(() => {
    ...
    // Aplicar regla 3/8 de Simpson
    // I = (3/8) * h * [f(a) + 3*sum(f(xi)) + f(b)]
    ...
    // Determinar cable correcto según el signo del resultado
    let correctColor = ''
    const tolerance = 0.00000001
    if (Math.abs(I) < tolerance) {
      correctColor = 'gray' // Resultado es cero
    } else if (I > 0) {
      correctColor = 'purple' // Resultado positivo
    } else {
      correctColor = 'orange' // Resultado negativo
    }

    console.log('=== Integración - Regla 3/8 de Simpson ===')
    console.log('Función: f(x) =', p.function)
    console.log('Límites: a =', p.a, ', b =', p.b)
    console.log('Segmentos: n =', p.n, '(múltiplo de 3)')
    console.log('Ancho: h =', h)
    console.log('Puntos calculados:')
    points.forEach((point, i) => {
      console.log(`  x${i} = ${point.x.toFixed(10)}, f(x${i}) = ${point.fx.toFixed(10)}`)
    })
    console.log('Integral I =', I)
    console.log('Signo del resultado:', I > tolerance ? 'Positivo' : I < -tolerance ? 'Negativo' : 'Cero')
  ...
  return (
    <ModuleScaffold
      {...props}
      topic="Integración"
      title="Regla 3/8 de Simpson"
      description="Calcula la integral usando la regla 3/8 de Simpson."
    >
      {/* Problema */}
      <div
        className={`space-y-4 rounded-lg border border-yellow-500/50 bg-yellow-500/5 p-4 mb-6 ${disabledClass}`}
      >
        <p className="text-sm font-bold">Regla 3/8 de Simpson</p>
      ...
      <div className={`grid grid-cols-2 gap-6 mb-6 ${disabledClass}`}>
        ...
        <div className="rounded-lg border border-red-500/50 bg-red-500/5 p-6">
          ...
          <div className="flex flex-col items-center gap-10">
            ...
            <CableVisual
              color="orange"
              isCut={cutCable === 'orange'}
              onClick={() => handleCutCable('orange')}
              disabled={cablesDisabled}
            />
        ...
        <div className="rounded-lg border border-white/10 bg-black/30 p-3 text-xs text-white/70">
          ...
          <p>
            ...
            <span className="text-emerald-300">
              {Math.abs(problem.I) < 0.00000001 ? 'Cero' : problem.I > 0 ? 'Positivo' : 'Negativo'}
            </span>
          </p>
        </div>
      )}
