...
export const IntegracionSimpsonUnTercioModule = (props) => {
  ...
  // Pool de 10 problemas predefinidos (n debe ser par)
  const problemsPool = [
    {
      function: '1 - x²',
      fnLatex: '1 - x^2',
  ...
  useEffect(() => {
    ...
    // I = (h/3) * [f(a) + 4*Σ(i=1,3,5...) + 2*Σ(i=2,4,6...) + f(b)]
    let sum = points[0].fx + points[points.length - 1].fx

    // Sumar términos con índice impar (multiplicar por 4)
    for (let i = 1; i < points.length - 1; i += 2) {
      sum += 4 * points[i].fx
    }

    // Sumar términos con índice par (multiplicar por 2)
    ...
    // Determinar cable correcto comparando parte entera con n
    const integerPart = Math.floor(Math.abs(I))
    let correctColor = ''

    if (integerPart > p.n) correctColor = 'yellow'
    else if (integerPart < p.n) correctColor = 'white'
    else correctColor = 'black'

    console.log('=== Integración - Regla 1/3 de Simpson ===')
  ...
  return (
    <ModuleScaffold
      {...props}
      topic="Integración"
      title="Regla 1/3 de Simpson"
      description="Calcula la integral usando la regla de Simpson 1/3."
    >
      {/* Problema */}
      <div
        className={`space-y-4 rounded-lg border border-yellow-500/50 bg-yellow-500/5 p-4 mb-6 ${disabledClass}`}
      >
        <p className="text-sm font-bold">Regla 1/3 de Simpson</p>
        ...
        <div className="text-xs text-white/70">
          ...
          <p>
            Número de intervalos: n = <span className="font-semibold text-white">{problem.n}</span>
          </p>
        </div>
      </div>
      ...
      {isCompleted && (
        <div className="rounded-lg border border-white/10 bg-black/30 p-3 text-xs text-white/70">
          ...
          <p>
            Parte entera = <span className="text-emerald-300">{problem.integerPart}</span>
          </p>
        </div>
      )}
    </ModuleScaffold>
