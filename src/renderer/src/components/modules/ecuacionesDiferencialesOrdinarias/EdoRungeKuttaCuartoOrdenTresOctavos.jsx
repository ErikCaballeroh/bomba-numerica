...
export const EdoRungeKuttaCuartoOrdenTresOctavosModule = (props) => {
  ...
  useEffect(() => {
    ...
    const k2 = h * eval(fExpression.replace('y', (y0 + k1/3)).replace('t', (t0 + h/3)))
    
    // k3 = h * f(yn + k1/3 + k2/3, tn + 2h/3)
    const k3 = h * eval(fExpression.replace('y', (y0 + k1/3 + k2/3)).replace('t', (t0 + 2*h/3)))
    
    // k4 = h * f(yn + k1 - k2 + k3, tn + h)
    const k4 = h * eval(fExpression.replace('y', (y0 + k1 - k2 + k3)).replace('t', (t0 + h)))
    
    // y1 = y0 + 1/8 * (k1 + 3*k2 + 3*k3 + k4)
    const y1 = y0 + (1/8) * (k1 + 3*k2 + 3*k3 + k4)
    ...
    const k2_2 = h * eval(fExpression.replace('y', (y1 + k1_2/3)).replace('t', (t1 + h/3)))
    
    // k3 para segunda iteración
    const k3_2 = h * eval(fExpression.replace('y', (y1 + k1_2/3 + k2_2/3)).replace('t', (t1 + 2*h/3)))
    
    // k4 para segunda iteración
    const k4_2 = h * eval(fExpression.replace('y', (y1 + k1_2 - k2_2 + k3_2)).replace('t', (t1 + h)))
    
    // y2 = y1 + 1/8 * (k1_2 + 3*k2_2 + 3*k3_2 + k4_2)
    const y2 = y1 + (1/8) * (k1_2 + 3*k2_2 + 3*k3_2 + k4_2)

    // Determinar cable correcto según el término k1 + 3k2 + 3k3 + k4 de la primera iteración
    const term = k1 + 3*k2 + 3*k3 + k4
    let correctCable = ''
    
    if (Math.abs(term) < 0.00000001) {
      correctCable = 'yellow' // Cercano a cero
    } else if (term > 0) {
      correctCable = 'blue'   // Positivo
    } else {
      correctCable = 'red'    // Negativo
    }

    console.log('=== Runge-Kutta Cuarto Orden (3/8 Simpson) ===')
    console.log('Problema:', selectedProblem.description)
    console.log('Ecuación:', equation)
    console.log('y0:', y0, 'h:', h)
    console.log('Iteración 1 - k1:', k1, 'k2:', k2, 'k3:', k3, 'k4:', k4)
    console.log('Término k1+3k2+3k3+k4:', term)
    console.log('y1:', y1)
    console.log('Iteración 2 - k1:', k1_2, 'k2:', k2_2, 'k3:', k3_2, 'k4:', k4_2)
    console.log('y2:', y2)
  ...
  return (
    <ModuleScaffold
      {...props}
      topic="EDO"
      title="Runge-Kutta de cuarto orden"
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className={`rounded-lg border border-yellow-500/50 bg-yellow-500/5 p-4 mb-6 ${disabledClass}`}>
            <div className="text-sm text-center font-bold text-yellow-300 mb-4">
              Resolver usando Runge-Kutta de 4° Orden (3/8 Simpson)
            </div>
