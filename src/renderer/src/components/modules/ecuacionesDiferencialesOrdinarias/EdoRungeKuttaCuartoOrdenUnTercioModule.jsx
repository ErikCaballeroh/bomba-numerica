...
export const EdoRungeKuttaCuartoOrdenUnTercioModule = (props) => {
  ...
  useEffect(() => {
    ...
    const k3 = h * eval(fExpression.replace('y', (y0 + k2/2)).replace('t', (t0 + h/2)))
    
    // k4 = h * f(yn + k3, tn + h)
    const k4 = h * eval(fExpression.replace('y', (y0 + k3)).replace('t', (t0 + h)))
    
    // y1 = y0 + 1/6 * (k1 + 2*k2 + 2*k3 + k4)
    const y1 = y0 + (1/6) * (k1 + 2*k2 + 2*k3 + k4)
    ...
    const k3_2 = h * eval(fExpression.replace('y', (y1 + k2_2/2)).replace('t', (t1 + h/2)))
    
    // k4 para segunda iteración
    const k4_2 = h * eval(fExpression.replace('y', (y1 + k3_2)).replace('t', (t1 + h)))
    
    // y2 = y1 + 1/6 * (k1_2 + 2*k2_2 + 2*k3_2 + k4_2)
    const y2 = y1 + (1/6) * (k1_2 + 2*k2_2 + 2*k3_2 + k4_2)

    // Determinar cable correcto según el término k1 + 2k2 + 2k3 + k4 de la primera iteración
    const term = k1 + 2*k2 + 2*k3 + k4
    let correctCable = ''
    
    if (Math.abs(term) < 0.00000001) {
      correctCable = 'blue'   // Cercano a cero
    } else if (term > 0) {
    ...
    console.log('=== Runge-Kutta Cuarto Orden (1/3 Simpson) ===')
    ...
    console.log('Término k1+2k2+2k3+k4:', term)
    console.log('y1:', y1)
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className={`rounded-lg border border-yellow-500/50 bg-yellow-500/5 p-4 mb-6 ${disabledClass}`}>
            <div className="text-sm text-center font-bold text-yellow-300 mb-4">
              Resolver usando Runge-Kutta de 4° Orden (1/3 Simpson)
            </div>
