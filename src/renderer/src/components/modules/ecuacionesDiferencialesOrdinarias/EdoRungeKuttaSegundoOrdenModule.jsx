...
export const EdoRungeKuttaSegundoOrdenModule = (props) => {
  ...
  useEffect(() => {
    ...
    // k2 = h * f(yn + k1, tn + h)
    const k2 = h * eval(fExpression.replace('y', (y0 + k1)).replace('t', (t0 + h)))
    
    // y1 = y0 + 1/2 * (k1 + k2)
    const y1 = y0 + 0.5 * (k1 + k2)
    ...
    const k2_2 = h * eval(fExpression.replace('y', (y1 + k1_2)).replace('t', (t1 + h)))
    
    // y2 = y1 + 1/2 * (k1_2 + k2_2)
    const y2 = y1 + 0.5 * (k1_2 + k2_2)
    ...
    if ((k1 >= 0 && k2 >= 0) || (k1 < 0 && k2 < 0)) {
      ...
      correctCable = 'yellow' // Signos opuestos
    }

    console.log('=== Runge-Kutta Segundo Orden ===')
    console.log('Problema:', selectedProblem.description)
    console.log('Ecuación:', equation)
    console.log('y0:', y0, 'h:', h)
    console.log('Iteración 1 - k1:', k1, 'k2:', k2, 'y1:', y1)
    console.log('Iteración 2 - k1:', k1_2, 'k2:', k2_2, 'y2:', y2)
    console.log('Signos - k1:', k1 >= 0 ? '+' : '-', 'k2:', k2 >= 0 ? '+' : '-')
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className={`rounded-lg border border-yellow-500/50 bg-yellow-500/5 p-4 mb-6 ${disabledClass}`}>
            <div className="text-sm text-center font-bold text-yellow-300 mb-4">
              Resolver usando Runge-Kutta de 2° Orden
            </div>
