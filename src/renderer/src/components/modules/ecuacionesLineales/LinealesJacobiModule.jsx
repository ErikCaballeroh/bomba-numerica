...
export const LinealesJacobiModule = (props) => {
  ...
  useEffect(() => {
    ...
    console.log('=== Ecuaciones Lineales - Método de Jacobi ===')
    console.log('Sistema:')
    console.log(
    ...
    while (errorA > tolerance || errorB > tolerance || errorC > tolerance) {
      ...
      // Calcular nuevos valores usando valores ANTERIORES (Jacobi)
      a = (p.eq1.b1 - p.eq1.a12 * b_prev - p.eq1.a13 * c_prev) / p.eq1.a11
      b = (p.eq2.b2 - p.eq2.a21 * a_prev - p.eq2.a23 * c_prev) / p.eq2.a22
      c = (p.eq3.b3 - p.eq3.a31 * a_prev - p.eq3.a32 * b_prev) / p.eq3.a33
  ...
  return (
    <ModuleScaffold
      ...
      description="Resuelve el sistema de ecuaciones usando el método de Jacobi."
    >
      {/* Problema */}
      <div
        className={`space-y-4 rounded-lg border border-yellow-500/50 bg-yellow-500/5 p-4 mb-6 ${disabledClass}`}
      >
        <p className="text-sm font-bold">Método de Jacobi</p>
        ...
        <div className="text-xs text-white/70 font-mono">
          ...
          <p>Valores iniciales: a₀ = 1, b₀ = 1, c₀ = 1</p>
        </div>
      </div>
