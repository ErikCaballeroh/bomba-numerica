...
export const MinimosCuadradosCuadraticaFuncionModule = (props) => {
  ...
  useEffect(() => {
    ...
    // Calcular todas las columnas necesarias (12 columnas)
    let sumX = 0, sumY = 0, sumX2 = 0, sumX3 = 0, sumX4 = 0
    let sumFx = 0, sumXFx = 0, sumX2Fx = 0, sumFx2 = 0
    let sumXY = 0, sumX2Y = 0, sumYFx = 0
    ...
    const A = [
      [n, sumX, sumX2, sumFx],
      [sumX, sumX2, sumX3, sumXFx],
      [sumX2, sumX3, sumX4, sumX2Fx],
      [sumFx, sumXFx, sumX2Fx, sumFx2]
    ]

    const b = [sumY, sumXY, sumX2Y, sumYFx]
    ...
    const gx = a0 + a1 * lastX + a2 * lastX ** 2 + a3 * lastFx

    // Determinar cable correcto según signo
    const correctColor = gx >= 0 ? 'orange' : 'gray'

    console.log('=== Mínimos Cuadrados Cuadrática con Función ===')
    ...
    console.log('g(x) =', gx)
    console.log('Signo: g(x) es', gx >= 0 ? 'positivo (+)' : 'negativo (-)')
  ...
  return (
    <ModuleScaffold
      ...
      title="Cuadrática con función"
      description="Ajuste cuadrático con transformaciones de función."
    >
      {/* Problema */}
      <div className={`space-y-4 rounded-lg border border-yellow-500/50 bg-yellow-500/5 p-4 mb-6 ${disabledClass}`}>
        <p className="text-sm font-bold">Ajuste cuadrático con f(x) = {problem.functionDisplay}</p>
      ...
      {isCompleted && (
        <div className="rounded-lg border border-white/10 bg-black/30 p-3 text-xs text-white/70">
          ...
          <p>Signo: <span className="text-emerald-300">{problem.gx >= 0 ? 'Positivo (+)' : 'Negativo (-)'}</span></p>
        </div>
      )}
    </ModuleScaffold>
  )
